...
function socket_connect() {
	...
	var socket_address = "https://bot-uptime.herokuapp.com:55839";
	...
	socket.on('error', function (error_message){
		if (error_message == "Gửi mail cho chúng tôi") {
			ui_show_error("Đăng ký tài khoản mới", "Gửi mail cho chúng tôi");
			if (current_page != 4) {
				show_page(4);
			}
		} 
		...
		if (!$.cookie('register') && error_message == "Not authorized") {
			if (current_page == 1) {
				ui_show_error("Login", "Oops, tài khoản hoặc mật khẩu của bạn không đúng");
			} else {
				show_page(1);
			}
		}  
	...
	socket.on('steambot steam game keys', function(result) {
		...
		var button_atrr = "";
		var button_text = "Thêm tài khoản steam";

		if (_steam_status == false) {
			button_atrr = "disabled";
			button_text = "Bạn chưa đăng nhập tài khoản steam";
		} else {
			button_atrr = "";
		}
		...
		for(var i = 0; i < keys.length; i++) {
			...
			if (apps_owned.indexOf(game_name) != - 1) {
				button_text = "Bạn đang sở hữu game";
				button_atrr = "disabled";
			}
...
function add_key_on_steam() {
	...
	if (code) {
		...
		setTimeout(function() {
			ui_show_error('Oops', 'Bạn không có trò chơi');
		}, 400);
	}
}
...
function add_steam_acc() {
	...
	if (login && password) {
		...
		setTimeout(function() {
			ui_show_error('Oops', 'Bạn phải nhập tên đăng nhập và mật khẩu tài khoản Steam của bạn');
		}, 400);
	}
}
...
function send_steamguard() {
	...
	if (code) {
		...
		setTimeout(function() {
			ui_show_error('Oops', 'Mã SteamGuard không tồn tại');
		}, 400);
	}
